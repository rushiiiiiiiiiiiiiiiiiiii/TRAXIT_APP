...
export default function DoctorLoginScreen({ navigation }) {
  ...
  useEffect(() => {
    const getAsync = async () => {
      const id = await AsyncStorage.getItem('userRole')
      console.log(id)
      getAsync()
    }
  }, [])
  ...
  const handleLogin = async () => {
    ...
    try {
      const result = await axios.post("http://192.168.0.124:8000/doclog", {
        phone,
        password,
      });
      ...
      if (result.data.success) {
        ...
        Alert.alert("Success", "Doctor Logined Successfully!");
        if (role == 'doctor') {
          navigation.dispatch(
            CommonActions.reset({
              index: 0,
